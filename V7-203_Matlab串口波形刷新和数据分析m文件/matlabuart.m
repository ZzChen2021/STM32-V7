...
%打开串口COM7，波特率115200，8位数据位，1位停止位，无奇偶校验，无流控制
...
SOF = 0;             %同步帧标志
AxisValue = 1;       %坐标值
RecDataDisp = zeros(1,10000000); %开辟100000个数据单元，用于存储接收到的数据。
RecData = zeros(1,10000);        %开辟100个数据单元，用于数据处理。
Axis = zeros(1,10000000);        %开辟100000个数据单元，用于X轴。
